...
Thank you for your interest in contributing to Loomind Studio! This document provides guidelines and instructions for contributing.
...
```bash
git clone https://github.com/KhaiTrang1995/loomind-studio.git
cd loomind-studio
```
...
> Running `npm install` installs these automatically (via the `prepare` script).
> The hooks block secrets on every commit:
>
> - **No secrets / no `.env` files** — a dependency-free secret scanner plus
>   `detect-private-key` scan staged content, and a guard refuses any real
>   `.env` file (templates such as `.env.example` are allowed).
>
> Never bypass with `--no-verify`. To clean the whole tree once before adopting
> the optional Python style hooks, run `npm run hooks:run`.
